...
public class Database 
{
	...
	private String createInsertCode()
	{
		...
		for(int i=0;i<tablesCounter;i++)
		{
			...
			{
				numberOfData = tables.get(i).generateReferenceValuesCombinations(this);
				if(tables.get(i).getNumberOfData()<=numberOfData)
					numberOfData = tables.get(i).getNumberOfData();
			}
			else
				numberOfData = tables.get(i).getNumberOfData();
